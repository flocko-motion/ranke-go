...
// MinHops is Min with its default of one hop applied. A negative Min is refused by
// ValidateQuery, which every read passes, so there is nothing here to fall back to.
