...
Alice is the **initial node** of a fresh Ranke-Archive. Her contributor claim carries her Ed25519 public key in its own content — the §5.7 special case where the initial node's pubkey lives in `v.content` rather than in a `contribution/contributor` edge (which has nothing to reference yet). She then ingests a single email as a `source/email` claim, signed by her key.

This is the foundational scenario: it exercises signing, the initial-node bootstrap, single-contributor attribution, and source ingestion, in the smallest possible graph.
