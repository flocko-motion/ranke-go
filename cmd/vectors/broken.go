...
func (g *gen) identitySign(raw []byte, id ranke.Id) error {
	...
	return g.addBroken("rejected-identity-sign", raw, hash.String(), reasonIdentitySign,
...
func (g *gen) malformedID(raw []byte, id ranke.Id) error {
	s := id.String()
	return g.addBroken("rejected-malformed-id", raw, s[:len(s)-6], reasonMalformedID,
...
func (g *gen) tamperedContent(id ranke.Id) error {
	...
	return g.addBroken("rejected-tampered-content", raw, id.String(), reasonIDMismatch,
...
func (g *gen) wrongHeight() error {
	...
	return g.addBroken("rejected-wrong-height", raw, c.ID().String(), reasonHeightWrong,
...
func (g *gen) unresolvableContributor(ctx context.Context) error {
	...
	return g.addBroken("rejected-absent-contributor", raw, c.ID().String(), reasonNoContributor,
...
func (g *gen) tamperedBlob() error {
	...
	return g.addContent("rejected-external-blob", []byte("tampered content, same declared hash"),
		hash, false, reasonContentMismatch,
