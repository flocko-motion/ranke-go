...
// type:    adapter
// job:     a graph-native CACHE Universe on neo4j — stores claim structure (nodes, edges) so
// closure/membership run as native Cypher instead of edge walks
// limits:  pure cache — inline content capped at 4 KiB; stack over a durable Universe
// (-> adapter/s3, adapter/fs) for canonical CBOR and external content
// dev:     `services/neo4j.sh native up` — bolt://127.0.0.1:7687, neo4j/rankeperfpass; the adapter
// tests auto-detect a reachable instance
package neo4j
