...
// map[string][]byte exposed as an adapter.BlobStore. The three primitives
// (Get/Put/Has) plus Close are all a byte-oriented backend has to provide;
// adapter.NewBlobUniverse supplies the entire ranke.Universe — claim codec,
...
func New() ranke.Universe {
	return adapter.NewBlobUniverse(&store{m: make(map[string][]byte)})
}
