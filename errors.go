...
var (
	...
	ErrQueryHeightUniverse   = errors.New("ranke.Query: Select.Height is meaningless under $universe (a universe has no revisions — it just is)")
	ErrQueryUnanchored       = errors.New("ranke.Query: unanchored traversal — Select.Path with no Select.Claim")
	ErrQueryHeightAhead      = errors.New("ranke.Query: Select.Height is above this archive's head — it names a revision the snapshot does not hold")
